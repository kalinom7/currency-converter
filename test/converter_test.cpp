...
void test_is_converted_amount_valid(std::string input_currency , std::string output_currency , double input_amount , double expected_converted_amount, int &errCnt )
{
    ExchangeRates exchange_rates = ExchangeRates();
    auto converter = Converter(exchange_rates, input_amount, input_currency , output_currency);

    auto result = converter.convert() == expected_converted_amount;

    if(!result)
    {
        std::cerr<<"Test failed for: "<<input_currency<<" "<<output_currency<<std::endl;
        errCnt++;
    }
}
